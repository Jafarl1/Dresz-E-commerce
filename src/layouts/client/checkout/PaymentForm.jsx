...
import { CARD_HOLDER, PAN, EXP_DATE, CVV, BANK } from "../../../utils/db";
...
export default function PaymentForm({ handleForm, setPaymentDetails }) {
  ...
  const handleCardNumberChange = (event) => {
    ...
    if (value.length <= 16) {
      ...
      handleForm(setPaymentDetails, PAN, formattedValue);
    }
  };
  ...
  const handleCvvChange = (event) => {
    ...
    if (value.length <= 3) {
      setCvv(value);
      handleForm(setPaymentDetails, CVV, value);
    }
  };
  ...
  const handleExpirationDateChange = (event) => {
    ...
    if (value.length <= 4) {
      ...
      handleForm(setPaymentDetails, EXP_DATE, formattedValue);
    }
  };

  const handleCardHolderName = (event) => {
    setCardHolder(event.target.value);
    handleForm(setPaymentDetails, CARD_HOLDER, event.target.value);
  };

  return (
    <Stack spacing={{ xs: 3, sm: 6 }} useFlexGap className="p-6">
          ...
          >
            <CardActionArea
              onClick={() => {
                ...
                handleForm(setPaymentDetails, BANK, false);
              }}
            >
              <CardContent
          ...
          >
            <CardActionArea
              onClick={() => {
                ...
                handleForm(setPaymentDetails, BANK, true);
              }}
            >
              <CardContent
                sx={{ display: "flex", alignItems: "center", gap: 1 }}
